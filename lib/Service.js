...
const TUA_MP_FILE_NAME = 'mvue.config.js'
...
module.exports = class Service {
  ...
  resolveWebpackConfig() {
    this.webpackChainFns.forEach((fn) => {
      fn(webpackConfig, this.tools, this.projectOptions)
    })
  ...
  genTools() {
    ...
    const getNameUrlObj = (url) => {
      ...
      // 入口是 main.js
      if (fs.existsSync(resolve(url, 'main.js'))) {
        return {[getRelativePath(url)]: [url]}
      }
    ...
    function getEntry (rootSrc, dir) {
      var map = {};
      glob.sync(path.join(rootSrc, dir) + '/**/*.vue')
      .forEach(file => {
        var key = file.replace(path.join(rootSrc, '/'), '').replace('.vue', '');
        map[key] = [resolve(file).replace('.vue', '')];
      })
      return map;
    }
    ...
    const getNameByFilePath = (f) => {
      const a = compose(
        file => path.relative(resolve('src'), file),
